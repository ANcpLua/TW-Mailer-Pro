...
public:
    ...
    MOCK_METHOD (void, sendCommand, (const std::string &command, bool debug),
    (override)
    ...
    MOCK_METHOD (std::string, getMaskedInput, (const std::string &prompt),
    (override));
    MOCK_METHOD (std::string, getMultiLineInput, (const std::string &prompt),
    (override));

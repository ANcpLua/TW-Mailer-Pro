#ifndef MOCKTWMAILERSERVER_H
#define MOCKTWMAILERSERVER_H
...
namespace testing
{
    ...
    public:
        ...
        MOCK_METHOD (bool, authenticate,
                (const std::string &username, const SecureString &password,
                 const std::string &clientIp),
        (override)

        );

        MOCK_METHOD (void, createUserDirectory, (const std::string &username),
        (override)

        );

        MOCK_METHOD (int, getRemainingAttempts, (const std::string &clientIp),
        (const, override));
    };
    ...
    public:
        ...
        MOCK_METHOD (std::vector<std::string>, listMails, (const std::string &user),
        (const, override));

        MOCK_METHOD (std::string, readMail, (const std::string &user, size_t mailId),
        (const, override));

        MOCK_METHOD (bool, deleteMail, (const std::string &user, size_t mailId),
        (override)
    ...
    public:
        ...
        MOCK_METHOD (void, handleCommand,
                (const std::string &command, ClientSession &session), (override)

        );

        MOCK_METHOD (void, handleLogin,
                (const std::vector<std::string> &args, ClientSession &session),
        (override)

        );

        MOCK_METHOD (void, handleSend,
                (const std::vector<std::string> &args, ClientSession &session),
        (override)

        );

        MOCK_METHOD (void, handleList,
                (const std::vector<std::string> &args, ClientSession &session),
        (override)

        );

        MOCK_METHOD (void, handleRead,
                (const std::vector<std::string> &args, ClientSession &session),
        (override)

        );

        MOCK_METHOD (void, handleDelete,
                (const std::vector<std::string> &args, ClientSession &session),
        (override)

        );

        MOCK_METHOD (void, handleQuit,
                (const std::vector<std::string> &args, ClientSession &session),
        (override)
    ...
    public:
        ...
        MOCK_METHOD (bool, isAuthenticated, (),
        (const, override));
        ...
        MOCK_METHOD (const std::string &, getUsername, (),
        (const, override));
        ...
        MOCK_METHOD (const std::string &, getClientIp, (),
        (const, override));
    };
